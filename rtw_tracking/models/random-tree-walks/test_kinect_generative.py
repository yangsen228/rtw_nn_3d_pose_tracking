...
def multitesting(y_test, previous_test_idx, joint_id, num_test, regressors, Ls, theta, k_idx, q1, q2):
            ...
            for xy_offset_idx in range(len(MAX_XY_OFFSET)):
                for round_idx in range(1):
                    ...
                    for kinem_idx, joint_id in enumerate(kinem_order):
                        ...
                        previous_test_idx = -1
                        p = Process(target = multitesting, args=(y_test,previous_test_idx,joint_id,num_test,regressors,Ls,theta,k_idx,q1,q2))
                        processes.append(p)
                        p.start()
