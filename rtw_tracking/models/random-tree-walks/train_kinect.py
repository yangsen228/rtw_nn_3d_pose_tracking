...
def get_random_offset(max_offset_xy, max_offset_z=MAX_Z_OFFSET):
...
def get_training_samples(joint_id, X, y, theta, num_samples, xy_offset, num_feats=NUM_FEATS):
    ...
    for train_idx in range(num_train):
        ...
        for sample_idx in range(num_samples):
            ...
            offset = get_random_offset(xy_offset)
...
def train(joint_id, X, y, model_dir, samples_leaf, k_value, num_samples, xy_offset):
    ...
    # Save models to disk
    folder = 'dl_%s_%d_%d_%d_%d/' % (TRAIN_SET, k_value, samples_leaf, num_samples, xy_offset)
...
def train_parallel(joint_id, X, y, theta, model_dir, regressor_queue, L_queue, samples_leaf, k_value, num_samples, xy_offset):
    """Train each joint in parallel.
    """
    S_f, S_u = get_training_samples(joint_id, X, y, theta, num_samples, xy_offset)
    regressor, L = train(joint_id, S_f, S_u, model_dir, samples_leaf, k_value, num_samples, xy_offset)
                    ...
                    if args.multithread:
                        ...
                        for joint_id in range(NUM_JOINTS):
                            p = Process(target=train_parallel, name='Thread #%d' % joint_id, args= \
                                        (joint_id, X_train, y_train, theta, args.model_dir, regressor_queue, L_queue, MIN_SAMPLES_LEAF[leaf_idx], K[k_idx], NUM_SAMPLES[samples_idx], MAX_XY_OFFSET[xy_offset_idx]))
                            processes.append(p)
                            p.start()
