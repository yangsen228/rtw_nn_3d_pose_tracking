...
parser.add_argument('--input-dir', type=str, default='/home/fusion/titech/depth-pose-estimation/data/processed',
...
args.model_dir = '/home/fusion/titech/depth-pose-estimation/output/random-tree-walks/' + args.dataset + '/models'
args.preds_dir = '/home/fusion/titech/depth-pose-estimation/output/random-tree-walks/' + args.dataset + '/preds'
args.png_dir = '/home/fusion/titech/depth-pose-estimation/output/random-tree-walks/' + args.dataset + '/png'
...
def load_dataset(processed_dir, is_mask=False, small_data=False):
    ...
    depth_images = np.load(os.path.join(processed_dir, 'kinect_depth_images_011.npy')) # N x H x W depth images
    joints = np.load(os.path.join(processed_dir, 'kinect_joints_011.npy')) # N x NUM_JOINTS x 3 joint locations
...
def stochastic(regressor, features, unit_directions, k_value):
    ...
    for leaf_id in leaf_ids:
        kmeans = MiniBatchKMeans(n_clusters=k_value, batch_size=400)
...
def train(joint_id, model_dir, samples_leaf, k_value):
    ...
    regressor_path = os.path.join(model_dir, 'plus_011/regressor' + str(joint_id) + '.pkl')
    L_path = os.path.join(model_dir, 'plus_011/L' + str(joint_id) + '.pkl')
...
def train_series(joint_id, X, y, theta, model_dir, samples_leaf, k_value):
    ...
    regressor, L = train(joint_id, model_dir, samples_leaf, k_value)
    return regressor, L
