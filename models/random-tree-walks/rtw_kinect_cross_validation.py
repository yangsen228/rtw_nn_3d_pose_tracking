...
def load_dataset(processed_dir, is_mask=False, small_data=False):
    ...
    depth_images = np.load(os.path.join(processed_dir, 'kinect_depth_images_011_020.npy')) # N x H x W depth images
    joints = np.load(os.path.join(processed_dir, 'kinect_joints_011_020.npy')) # N x NUM_JOINTS x 3 joint locations
...
def train(joint_id, X, y, model_dir, samples_leaf, k_value):
    ...
    # Save models to disk
    regressor_path = os.path.join(model_dir, '011-020/regressor' + str(joint_id) + '.pkl')
    L_path = os.path.join(model_dir, '011-020/L' + str(joint_id) + '.pkl')
